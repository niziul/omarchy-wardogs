...
// The site's own side-by-side comparison page. It server-renders the full
// stat blobs of every listed id (any kind — the per-item pages embed stats
// only for weapon-family items), so one fetch resolves a whole compare pair.
function compareUrl(ids) {
  var clean = [];
  for (var i = 0; i < (ids || []).length; i++) {
    var s = sanitizeId(ids[i]);
    if (s !== "") clean.push(s);
  }
  if (clean.length === 0) return "https://wardogs.zone/database/compare";
  return "https://wardogs.zone/database/compare?items=" + clean.join(",");
}
...
var SITE_LINKS = [
  { label: "Loadouts", url: "https://wardogs.zone/loadouts", glyph: "\uF0B1", icon: "wardogs-grid.svg" },
  { label: "Calculators", url: "https://wardogs.zone/calculators/damage", glyph: "\uF1EC", icon: "wardogs-damage-calc.svg" },
  {
    label: "Base Builder",
    url: "https://wardogs.zone/loadouts/base",
    glyph: "\uF286",
    icon: "wardogs-base.svg",
  },
  { label: "Maps", url: "https://wardogs.zone/maps/kavkazi", glyph: "\uF279", icon: "wardogs-maps.svg" },
  { label: "Wiki", url: "https://wardogs.zone/wiki", glyph: "\uF02D", icon: "wardogs-field-manual.svg" },
  { label: "Updates", url: "https://wardogs.zone/updates", glyph: "\uF1DA", icon: "wardogs-progression.svg" },
  { label: "Community", url: "https://wardogs.zone/community", glyph: "\uF0C0", icon: "wardogs-hub.svg" },
];
